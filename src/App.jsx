...
import Navbar from './fl/navbar/Navbar'
import NavFooter from './fl/navFooter/NavFooter'
import NavPath from './fl/Page/NavPath';
import useImageStore from './fl/api/ImageLink';
import Image from './fl/user/Image';
...
export default function App() {
  ...
  let search = loc.slice(7,length)
  let locol =  sessionStorage.getItem('local')
  const {image, getImg} = useImageStore()

  if(locol){
    useEffect(()=>{
      for (let i = 0; i < link.length; i++) {
       if(link[i].pathName === locol){
        setPath(link[i])
        getImg({searchImage:link[i]?.pathName})
       }
  ...
  //   if(path !==''){
  //     let id = path.id - 1
  //     // setSearchParams({query:item?.pathName})
  //     // useEffect(()=>{
  //       console.log(id)
  //       // for(let i = 0; i < link.length; i++){
  //         console.log(link[id])
  //         if(link[id].pathName === path.pathName){
  //           navigate('/page')
            // setPageItem(path)
  //           console.log(link[id], 'path')
            // setSearchParams({query:path?.pathName})
  ...
  return (
    <div className='w-[100vw] '>
      <div className=' sticky top-0 z-50'>
        <Navbar/>
        <NavFooter value={link} path={setPath} />
      </div>
      <Routes>
        <Route path='/' element={<Image/>}/>
        <Route path='/page' element={<NavPath value={path}  />}/>
      </Routes>
    </div>
  )
}
